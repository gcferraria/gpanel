    ...
    public function index( $id ) {
        ...
        foreach ( $fields as $field ) {
            $data[] = array(
                ...
                4 => '<a href="'.site_url('content_types/fields/edit/'. $field->id).'" class="btn btn-xs green-meadow"><i class="fa fa-edit"></i> Editar</a>
                      <a href="#" class="btn btn-xs red-sunglo" data-text="Tem a certeza que pretende apagar o registo?" data-url="'.base_url('content_types/fields/delete/' . $field->id ).'.json" data-jsb-class="App.DataTable.Delete"><i class="fa fa-trash-o"></i> Apagar</a>',
            );
        }
    ...
    **/
    public function add( $id ) {
        ...
        if ( $content_type_field->save( $content_type ) ) {
            ...
            $data = array(
                'reset'        => 1,
                'notification' => array( 'success', $this->lang->line('content_type_field_insert_success_message') )
            );
        }
        else {
            ...
            $data = array(
                ...
                'notification' => array( 'error', $this->lang->line('content_type_field_insert_error_message') ),
            );
        }
    ...
    **/
    public function edit( $id ) {
        ...
        if ( $content_type_field->save() ) {
            ...
            $data = array(
                'show_errors'  => array(),
                'notification' => array( 'success', $this->lang->line('content_type_field_update_success_message') )
            );
        }
        else {
            ...
            $data = array(
                'show_errors' => $content_type_field->errors->all,
                'notification' => array( 'error', $this->lang->line('content_type_field_update_error_message') ),
            );
        }
        ...
        */
        if ( $content_type_field->delete() ) {
            return parent::index(
                array(
                    'result'  => 1,
                    'message' => $this->lang->line('content_type_field_delete_success_message'),
                )
            );
        }
        else {
            return parent::index(
                array(
                    'result'  => 0,
                    'message' => $this->lang->line('content_type_field_delete_error_message'),
                )
            );
        }
    }
...
/* Location: ./applications/gpanel/controllers/json/content_types/fields.php */
