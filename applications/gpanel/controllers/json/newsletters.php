    ...
    public function index() {
        ...
        foreach ( $newsletters as $newsletter ) {
            $data[] = array(
                ...
                2 => '<a href="'.base_url('newsletters/open/'. $newsletter->id).'.json" class="btn btn-xs blue-madison" data-toggle="modal" data-target="#newsletterModal">'
                    .'<i class="fa fa-search"></i> Abrir</a> '
                    .'<a href="#" class="btn btn-xs red-sunglo" data-text="Tem a certeza que pretende apagar o registo?" data-url="'.base_url('newsletters/delete/' . $newsletter->id ).'.json"'
                    .' data-jsb-class="App.DataTable.Delete"><i class="fa fa-trash-o"></i> Apagar</a>',
            );
        }
        ...
        if ( $newsletter->valid && $step == $steps ) {
            if ( $newsletter->save() ) {
                ...
                $data = array(
                    ...
                    'notification' => array('success', $this->lang->line('newsletter_insert_success') ),
                );
            }
            else {
                $data = array( 'notification' => array('error', $this->lang->line('newsletter_insert_error') ) );
            }
        }
        else {
        ...
        */
        if ( $newsletter->delete() ) {
            return parent::index(
                array(
                    'result'  => 1,
                    'message' => $this->lang->line('newsletter_delete_success'),
                )
            );
        }
        else {
            return parent::index(
                array(
                    'result'  => 0,
                    'message' => $this->lang->line('newsletter_delete_error'),
                )
            );
        }
    }
...
/* Location: ./applications/gpanel/controllers/json/newsletters.php */