...
class Users extends JSON_Controller {
    ...
    /**
     * __construct: Ajax Users Class constructor.
     *
     * @access public
     * @return void
    **/
    ...
    public function index() {
        ...
        foreach ( $users as $user ) {
            $data[] = array(
                ...
                4 => '<a href="'.site_url('users/edit/'. $user->id).'" class="btn default btn-xs green-meadow"><i class="fa fa-edit"></i> Editar</a>',
            );
        }
    ...
    public function add() {
        ...
        if ( $user->valid ) {
            ...
            if ( $user->save( array( 'roles' => $roles ) ) ) {
                $data = array(
                    'reset'        => 1,
                    'notification' => array('success', $this->lang->line('user_insert_success_message') ),
                );
            }
        }
        else {
            ...
            $data = array(
                ...
                'notification' => array('error', $this->lang->line('user_insert_error_message') ),
            );
        }
    ...
    public function edit( $id ) {
        ...
        if ( $user->valid ) {
            ...
            if ( $user->save( array( 'roles' => $roles ) ) ) {
                $data = array(
                    'show_errors'  => array(),
                    'notification' => array('success', $this->lang->line('user_update_success_message') ),
                );
            }
        }
        else {
            $data = array(
                'show_errors'  => $user->errors->all,
                'notification' => array('error', $this->lang->line('user_update_error_message') ),
            );
        }
